...
export default function EventControlPanel({ eventCategories }: ControlPanelProps) {
    ...
    return (
        <div className="relative overflow-visible py-8 px-4">
            {/* Holographic Projection overlay above the entire events section */}
            {selectedEvent && (
                <HolographicProjection event={selectedEvent} onClose={() => setSelectedEvent(null)} />
            )}
            ...
            <div className="relative z-10 max-w-7xl mx-auto">
                <div className="rounded-2xl overflow-hidden shadow-2xl flex flex-col lg:flex-row min-h-[400px] md:min-h-[600px] relative">
                    ...
                    <div className="absolute inset-0 pointer-events-none overflow-hidden rounded-2xl">
                        {/* Corner sparkles */}
                        <div className="absolute top-2 left-2 w-2 h-2 bg-white rounded-full opacity-0 animate-sparkle" style={{ animationDelay: '0s' }}></div>
                        <div className="absolute top-4 right-8 w-1 h-1 bg-gray-300 rounded-full opacity-0 animate-sparkle" style={{ animationDelay: '1.2s' }}></div>
                        <div className="absolute bottom-6 left-12 w-1.5 h-1.5 bg-white rounded-full opacity-0 animate-sparkle" style={{ animationDelay: '2.4s' }}></div>
                        <div className="absolute bottom-3 right-4 w-1 h-1 bg-gray-300 rounded-full opacity-0 animate-sparkle" style={{ animationDelay: '3.6s' }}></div>
                        <div className="absolute top-1/3 left-1 w-1 h-1 bg-white rounded-full opacity-0 animate-sparkle" style={{ animationDelay: '1.8s' }}></div>
                        <div className="absolute top-2/3 right-2 w-1.5 h-1.5 bg-gray-300 rounded-full opacity-0 animate-sparkle" style={{ animationDelay: '3s' }}></div>
                    </div>
                    {/* Left Control Panel - Hidden on mobile, 30% on desktop */}
                    <div className="hidden lg:block w-full lg:w-[30%] border-r border-gray-500/30 relative">
                        <div className="h-full border border-gray-500/40 rounded-lg m-2 bg-black/20 backdrop-blur-sm">
                            <SwitchPanel
    ...
    return (
        <div className="p-4 lg:p-6 h-full overflow-y-auto">
            ...
            <div className="text-center mb-6 lg:mb-8">
                <h2 className="text-white font-mono text-lg lg:text-xl font-bold mb-2">EVENT CONTROL</h2>
                <div className="w-full h-px bg-gradient-to-r from-transparent via-gray-500 to-transparent"></div>
                <p className="text-gray-300/70 font-mono text-xs mt-2">Toggle categories to activate</p>
            </div>
...
function CategorySwitch({ category, isActive, onToggle }: {
    ...
    return (
        <div
            onClick={onToggle}
            className={`p-3 lg:p-4 rounded-lg border transition-all duration-300 cursor-pointer relative overflow-hidden group ${isActive
                    ? 'bg-gray-500/10 border-gray-400/70'
                    : 'border-gray-500/60 hover:border-gray-400/80'
                }`}
        >
            {/* Base white ray effect for switch sections */}
            <div
                className="absolute inset-0 pointer-events-none"
                style={{
                    background: 'linear-gradient(45deg, transparent 30%, rgba(255,255,255,0.2) 50%, transparent 70%)',
                    animation: 'sectionWhiteRay 6s ease-in-out infinite',
                    transform: 'translateX(-100%)',
                    animationDelay: `${Math.random() * 4}s`
                }}
            ></div>

            {/* Enhanced hover white effect with higher intensity */}
            <div
                className="absolute inset-0 opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-300"
                style={{
                    background: 'linear-gradient(45deg, transparent 20%, rgba(255,255,255,0.5) 50%, transparent 80%)',
                    animation: 'sectionWhiteRay 2s ease-in-out infinite',
                    transform: 'translateX(-100%)'
                }}
            ></div>
...
function DisplayScreen({ activeCategories, selectedCategory, eventCategories, selectedEvent, onEventSelect, onBackToCategory, onCategorySelect, isMobile }: {
    ...
    // Only show CRT monitor for category/event lists, not for event details
    return (
                ...
                <div className="h-full overflow-auto">
                    ...
                    <div className="text-center mb-4 md:mb-6">
                        <h1 className="text-xl md:text-3xl font-bold mb-2 md:mb-3">
                            <span className="text-white font-mono">CONVERGENCE</span>{' '}
                            <span className="text-gray-300">2K25R</span>
                        </h1>
                        <div className="w-full h-px bg-gradient-to-r from-transparent via-gray-500 to-transparent mb-2 md:mb-3"></div>
                        <p className="text-gray-300/80 font-mono text-sm">
                    ...
                    {isMobile ? (
                        ...
                        <>
                            {/* System Status */}
                            <div className="mb-3 md:mb-4 p-2 md:p-3 border border-gray-400/30 rounded">
                                <div className="flex justify-between items-center mb-1 md:mb-2">
                                    <span className="text-white font-mono text-sm md:text-base">SYSTEM STATUS</span>
...
function CRTMobileAppLauncher({ eventCategories, onCategorySelect }: {
    ...
    return (
        <div className="space-y-4">
            <div className="text-white font-mono text-base font-bold mb-4 text-center">
                {'>'} EVENT CATEGORIES
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
                {eventCategories.map((category) => (
                    <div
                        ...
                        className="border border-gray-400/30 rounded p-4 cursor-pointer hover:border-gray-400/60 hover:bg-gray-500/10 transition-all text-center relative overflow-hidden"
                    >
                        {/* White ray effect for individual sections */}
                        <div
                            className="absolute inset-0 opacity-0 pointer-events-none"
                            style={{
                                background: 'linear-gradient(45deg, transparent 30%, rgba(255,255,255,0.15) 50%, transparent 70%)',
                                animation: 'sectionWhiteRay 8s ease-in-out infinite',
                                transform: 'translateX(-100%)',
                                animationDelay: `${Math.random() * 4}s`
                            }}
                        ></div>
                        ...
                        <div className="text-gray-300/70 text-xs mb-2 line-clamp-2">
                            {category.desc}
                        </div>
    ...
    return (
        <div className="flex items-center justify-center h-48">
            <div className="text-center">
                <div className="text-white font-mono text-xl mb-4 animate-pulse">
                    {'>'} AWAITING INPUT_
                </div>
                <p className="text-gray-300/60 font-mono text-base">
...
function CRTCategoryOverview({ categories }: { categories: any[] }) {
    return (
        <div className="space-y-3 md:space-y-4">
            <div className="text-white font-mono text-sm md:text-base font-bold mb-3 md:mb-4">
            ...
            <div className="space-y-2 md:space-y-3">
                {categories.map((category) => (
                    <div
                        key={category.id}
                        className="p-2 md:p-3 border border-gray-400/30 rounded bg-gray-500/5 relative overflow-hidden"
                    >
                        {/* White ray effect for switch sections (same intensity as monitor) */}
                        <div
                            className="absolute inset-0 opacity-0 pointer-events-none"
                            style={{
                                background: 'linear-gradient(45deg, transparent 30%, rgba(255,255,255,0.2) 50%, transparent 70%)',
                        ...
                        <div className="flex items-center justify-between mb-1 md:mb-2">
                            <div className="flex items-center space-x-2">
                                <div>
                                    <div className="text-white font-mono font-bold text-sm md:text-base">{category.title}</div>
                                    <div className="text-gray-300/70 text-sm">{category.desc}</div>
                                </div>
                            </div>
                            <div className="text-right">
                                <div className="text-white font-mono text-sm">[{category.count}]</div>
                                <div className="flex items-center space-x-1 justify-end mt-1">
                                    <div className="w-1 h-1 bg-white rounded-full animate-pulse"></div>
                                    <span className="text-white font-mono text-sm">ON</span>
                                </div>
                            </div>
                        </div>
                    </div>
...
function CRTCategoryDetailView({ category, events, onEventSelect, onBackToApps }: {
    ...
    return (
        <div className="space-y-3 md:space-y-4">
            ...
            {onBackToApps && (
                <div className="flex items-center justify-between mb-4">
                    <button
                        ...
                        className="text-white hover:text-gray-300 font-mono text-sm border border-gray-400/30 px-5 py-2 rounded hover:bg-gray-500/10 transition-all"
                    >
                        {'<'} BACK TO CATEGORIES
                    </button>
                </div>
            ...
            {/* Category Header */}
            <div className="border border-gray-400/30 rounded p-2 md:p-3 bg-gray-500/5">
                <div className="flex items-center space-x-2 mb-1 md:mb-2">
                    <div>
                        <div className="text-white font-mono font-bold text-sm md:text-base">{category.title}</div>
                        <div className="text-gray-300/70 text-xs">{category.desc}</div>
                    </div>
                    <div className="text-white font-mono text-xs">
            ...
            <div className="grid grid-cols-1 gap-2 md:gap-3">
                {events.map((event) => (
                    <React.Fragment key={event.id}>
                        <div className="border border-gray-400/20 rounded transition-all overflow-hidden bg-black/30 flex flex-col items-stretch p-0">
                            {/* Event Poster - full image */}
                            <div className="relative w-full h-64 md:h-80 overflow-hidden rounded-t-lg">
                                ...
                                <div className="absolute top-2 right-2">
                                    <div className="px-2 py-1 rounded text-xs font-mono font-bold bg-gray-800/90 text-white">
                                        [OPEN]
                                    </div>
                                </div>
                            </div>
                            {/* Bottom row: event name on left, compact buttons on right */}
                            <div className="flex flex-row items-center justify-between px-4 py-2 bg-black/40 rounded-b-lg gap-2">
                                <div className="font-mono text-sm font-bold text-white flex-1 min-w-0 pr-2">{event.name}</div>
                                <div className="flex flex-row flex-shrink-0" style={{ gap: '1px' }}>
                                    <button
                                        className="px-1.5 py-1 rounded bg-gray-800/90 text-white font-mono text-xs font-bold hover:bg-gray-700/90 transition"
                                        onClick={() => onEventSelect(event)}
                                    >
                                        More
                                    </button>
                                    <button
                                        className="px-1.5 py-1 rounded bg-gray-700/90 text-white font-mono text-xs font-bold hover:bg-gray-800/90 transition"
...
//         <button
//           onClick={onBack}
//           className="text-cyan-400 hover:text-cyan-300 font-mono text-sm border border-cyan-400/30 px-3 md:px-5 py-2 rounded hover:bg-cyan-500/10 transition-all"
//         >
//           {'<'} {showBackToEvents ? 'BACK TO EVENTS' : 'CLOSE DETAIL'}
//         </button>
//         <p className="text-cyan-300/70 font-mono text-xs">
//           EVENT_ID: {event.id}
//         </p>
//       </div>
...
//       {/* Title and Club */}
//       <div className="text-center mb-4 md:mb-6">
//         <h2 className="text-xl md:text-2xl font-bold font-mono text-cyan-400 mb-1">{event.name}</h2>
//         <p className="text-cyan-300/80 font-mono text-sm">Organized by: {event.club}</p>
//         <div className="w-full h-px bg-gradient-to-r from-transparent via-cyan-500/50 to-transparent mt-3"></div>
//       </div>
//
//       <div className="space-y-4 md:space-y-6">
//         {/* Description */}
//         <div className="border border-cyan-400/20 rounded p-3 bg-cyan-500/5">
//           <h3 className="text-cyan-400 font-mono text-sm font-bold mb-3">{'>'} DESCRIPTION</h3>
